...
abstract class Message {
  ...
  def toJson : String = generate(this)
}
...
class TSClientSender(var channel: SocketChannel) {
  ...
  def send(message: Message) = {
    ...
    val buffer = ByteBuffer.allocate(msgByteArray.length + 1)
    ...
    while(!finished) {
      receive {
        case CommandMessage(id, cmd, msg) =>
          /* Command invocation may be cascade,
...
class TSClientReceiver[CI <: TSClientInterface](var channel: SocketChannel,
  ...
  private def decodeMessage(buffer: ByteBuffer) = {
    ...
    val message = MessageParser.fromJson(msgStr)
    ...
    while(position < bytes.length) {
      ...
      if(bytes(position) == 0) {
        ...
        if(len > 0) {
          ...
          val msgBuffer = ByteBuffer.wrap(msgBytes)
  ...
  private def decodeByteBuffer(buffer: ByteBuffer, length: Int) {
    ...
    val chunkBytes = new Array[Byte](length)
...
class TSClient[CI <: TSClientInterface](var channel: SocketChannel, var server: TSServer[CI])  {
  ...
  val sender = new TSClientSender(channel)
  val receiver = new TSClientReceiver[CI](channel, this)
  val recvThread = new Thread(receiver)
  ...
  def disconnectFromServer(){
    server.processClientDisconnect(this)
  }
}